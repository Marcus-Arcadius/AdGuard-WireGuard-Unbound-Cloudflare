...
#### <a href="https://www.nlnetlabs.nl/projects/unbound/about/"><b><i>Свободный</i></b></a>с[<i>Стабби</i>](https://dnsprivacy.org/dns_privacy_daemon_-_stubby/about_stubby/): Проверяющий, рекурсивный, кэширующий преобразователь DNS
...
<i>Все программное обеспечение является бесплатным, с открытым исходным кодом и самостоятельный </i></br><a href="https://github.com/trinib/Adguard-Wireguard-Unbound-Cloudflare/wiki/About"><b>О</b></a>\|<a href="https://github.com/trinib/AdGuard-WireGuard-Unbound-Cloudflare/discussions/17"><b>Часто задаваемые вопросы</b></a>\|<a href="https://github.com/trinib/AdGuard-WireGuard-Unbound-Cloudflare/wiki"><b>Вики</b></a>                         
...
-   [Установите CloudFlare](#install-cloudflare)<img src="https://www.vectorlogo.zone/logos/cloudflare/cloudflare-icon.svg" width=20px height=20px>
...
После завершения перепрошивки найдите в «Этот компьютер» имя диска «загрузочный или USB-накопитель» (повторно подключите устройство чтения карт памяти USB, если его не видно). Перейдите на этот диск, создайте новый текстовый файл с именем**_`ssh without 'txt' extension`_**. Отключите «Скрыть расширения для известных типов файлов» в настройках проводника, если вы его не видите.
...
Введите «да» для вопроса об отпечатке пальца и введите «малина» для пароля по умолчанию (пароли будут невидимы в командной строке). Вы можете ввести**_`sudo passwd pi`_**сменить пароль после.
...
Мой черный список, например[<a href="https://github.com/trinib/AdGuard-WireGuard-Unbound-Cloudflare/blob/main/My-Blocklist.txt"><b>кликните сюда</b></a>]

**_Окончательные источники черных списков_**:</br><a href="https://github.com/T145/black-mirror"><img src="https://raw.githubusercontent.com/T145/black-mirror/master/.github/images/logo.png" width=220px height=60px></a>  
<a href="https://github.com/T145/black-mirror"><b>черное зеркало</b></a>-_Автоматически поддерживаемые черные списки вредоносных хостов и ложноположительные белые списки_</br>👊БОЛЬШОЕ СПАСИБО👊<a href="https://github.com/T145"><b>Т 145</b></a>
...
На данный момент вы можете добавлять только один URL-адрес в черный список DNS с помощью AdGuard, но есть скрипт Python для добавления нескольких URL-адресов одновременно.
...
Введите в командной строке`crontab -e`, он попросит выбрать редактор (выберите 1), вставьте эти строки внизу crontab и сохраните (control + x, затем y, затем введите):
...
# <i>Установите CloudFlare</b></i>

## Настройка для Cloudflared`(DoH)`

[<a href="https://github.com/trinib/AdGuard-WireGuard-Unbound-Cloudflare/wiki/Install-Cloudflared-service-with-DNS-over-HTTPS-proxy"><b>кликните сюда</b></a>]</h4>
...
-   Скрипт запросит у вас общедоступный IPv4/имя хоста для VPN._Если_у вас есть статический IP-адрес, затем продолжите или введите динамический домен DNS, который вы создали из<a href="https://github.com/trinib/AdGuard-WireGuard-Unbound-Cloudflare/wiki/Create-a-Dynamic-DNS-Hostname-Service"><b>инструкции</b></a>. Например: trinibvpn.freeddns.org

-   Для варианта порта`press enter`по умолчанию 51820. Для имени клиента просто введите любое имя, которое вы хотите, а для DNS используйте вариант 3 (`1.1.1.1`) теперь. Вы будете настраивать`AdGuard/Unbound/Cloudflare`с VPN после завершения его установки.
...
<b>Использовать_OpenVPN_</b>[<a href="https://github.com/trinib/AdGuard-WireGuard-Unbound-Cloudflare/wiki/Install-OpenVPN"><b>кликните сюда</b></a>]
...
`UPDATE:`После обновления WireGuard я не получаю более высокую скорость, делая это 😞 .. но это все еще имеет смысл_нет_использовать`"0.0.0.0/0, ::/0`с Wi-Fi. Если кто-нибудь знает какие-либо настройки, чтобы получить повышение, дайте мне знать.
...
#### Отключите IPv6, если у вас его нет или он вам не нужен[<a href="https://github.com/trinib/AdGuard-WireGuard-Unbound-Cloudflare/wiki/Disable-all-IPv6"><b>кликните сюда</b></a>]. В результате, если у вас слабый интернет, отключение IPv6 может ускорить запрос DNS, но снизить безопасность.
...
[хттпс://гитхов.ком/Т145/блацк-миррор](https://github.com/T145/black-mirror)
